<template>
    <div class="listfoot">
      <van-goods-action>
        <van-goods-action-mini-btn icon="chat-o" text="客服"/>
        <van-goods-action-mini-btn icon="star-o" text="收藏"/>
        <van-goods-action-mini-btn icon="cart-o" text="购物车" @click="onClickMiniBtn"/>
        <van-goods-action-big-btn text="加入购物车" @click="onClickBigBtn"/>
        <van-goods-action-big-btn primary text="立即购买"/>
      </van-goods-action>
    </div>
</template>

<script>
import Vue from "vue";
import {
  GoodsAction,
  GoodsActionBigBtn,
  GoodsActionMiniBtn
} from 'vant';

Vue
  .use(GoodsAction)
  .use(GoodsActionBigBtn)
  .use(GoodsActionMiniBtn);
export default {
    props: ["prolist"],
    // props:{
    //     prolist: Array,
    // },
    mounted () {
      console.log(this.prolist)  //对象
    },
    methods: {
        onClickMiniBtn(){
            this.$router.push("/cart");
        },
        onClickBigBtn() {
            let goods = JSON.parse(localStorage.getItem("goods")) || [];
            if (goods.length === 0) {
                goods.push({
                id: this.prolist.skuid,
                img: this.prolist.img,
                title: this.prolist.title,
                name: this.prolist.name,
                price: this.prolist.skuprice,
                num: 1
                });
            } else {
                let arr = [];
                arr = goods.filter(item => {
                return item.id === this.prolist.skuid;
                });
                if (arr.length === 0) {
                goods.push({
                    id: this.prolist.skuid,
                    img: this.prolist.img,
                    title: this.prolist.title,
                    name: this.prolist.name,
                    price: this.prolist.skuprice,
                    num: 1
                });
                } else {
                goods.map(item => {
                    if (item.id === this.prolist.skuid) {
                    item.num++;
                    }
                });
                }
            }

            localStorage.setItem("goods", JSON.stringify(goods));
            }
    }
}
</script>

